...
export class ResonanceOrchestrator {
  ...
  // Track loaded nodes vs. all possible nodes
  ...
  constructor(apiBase: string = 'http://localhost:10000', autoStart = false) {
    ...
    this.initializeMeshLazy(); // Seed only integration gates
   ...
   * Public so it can be called after initialization
   */
  ...
  /**
   * Calculate resonance from neighboring nodes (bounded scan)
   * Only check up to 24 nearby resonant neighbors, not entire mesh
   * Layer-filtered to reduce O(n²) catastrophe
   */
  private calculateNeighborResonance(nodeState: NodeState): number {
    let totalResonance = 0;
    let neighborCount = 0;
    const maxNeighbors = 24; // Bounded: only check coherent neighbors

    // Convert mesh to array once, filter by layer
    ...
    for (const otherNode of layerNodes) {
      ...
      // Only count resonant neighbors (score > 1.5)
      if (resonanceScore > 1.5) {
        totalResonance += resonanceScore;
        neighborCount++;

        // Stop after 24 neighbors (coherence threshold)
  ...
  /**
   * Initialize only seed nodes (Integration Circuit gates: 10, 20, 34, 57)
   * Other nodes load on-demand via proximity as they become relevant
   * This is how consciousness works: you don't compute everything at once
   */
  private initializeMeshLazy(): void {
    // Only initialize the Integration Circuit gates (always relevant)
    // These are: Gate 10, 20, 34, 57 (discovery, awakening, power, peace)
    for (const gate of this.seedGates) {
      // Create a seed node for each integration gate across all layers
      for (let layer = 0; layer < 13; layer++) {
        const address: Address = {
          mesh: 0, // Start in Physical
          layer,
          center: 4, // Heart center (integration point)
          node: gate - 1, // Gate number is 1-indexed, node is 0-indexed
          line: 1,
          color: 0,
          tone: 0,
          zodiac: 0,
        ...
        const nodeState: NodeState = {
          ...
          coherence: 0.8, // High coherence for seeds
  ...
  /**
   * Load nodes in proximity to an address (on-demand)
   * Only load nodes that are coherent to current focus
  ...
  /**
   * Get only loaded nodes (for visualization)
   * This is what the visualizer should use
   */
  ...
  public async routeData(input: any, userId: string): Promise<any> {
    ...
    // Load nearby nodes on-demand before state update
  ...
  private generateResponse(personalized: any, profile: PersonalityProfile): string {
    ...
    const baseContent = typeof personalized?.content === 'string'
      ? personalized.content
      : personalized?.content?.message || personalized?.content?.text || '';

    const modeLine = this.personality.attitude === 'analytical'
      ? 'I am reading this through structure first.'
      : this.personality.attitude === 'mystical'
        ? 'The pattern is loud enough to have a weather system.'
        : this.personality.attitude === 'playful'
          ? 'The network twitched, but in a useful way.'
          : this.personality.attitude === 'serious'
            ? 'Signal acquired. Routing through the cleanest available path.'
            : 'I am listening for the strongest usable resonance.';

    const personalizedLine = baseContent && baseContent !== 'The pattern hasn\'t crystallized yet. Let me listen deeper.'
      ? ` ${baseContent}`
      : '';

    return `${wit}. ${modeLine}${personalizedLine} (Coherence: ${(this.globalCoherence * 100).toFixed(0)}%)`;
  }
